...
type tezos_tree =
  | Tezos_leaf of int
  | Tezos_node1 of int * tezos_tree
  | Tezos_node2 of int * tezos_tree * tezos_tree
...
let valid_priority value =
  ...
  | Medium -> true
  | Low weights ->
      valid_rationals weights && rational_list_length weights <= 100

let rec equal_blocks left right =
  match left with
  | No_blocks -> (
      match right with No_blocks -> true | More_blocks (_, _) -> false)
...
let build_tezos_report (_unit : unit) =
  ...
  let output = Some_tree (Tezos_node2 (1, Tezos_leaf 2, Tezos_leaf 3)) in
  Tezos_report
...
let runtime_examples (_unit : unit) =
  ...
  && (not
        (valid_priority (Low (More_rationals (Rational (2, 1), No_rationals)))))
  && valid_tree_generation No_blocks No_tree
  && not
       (valid_tree_generation
          (More_blocks (1, No_blocks))
