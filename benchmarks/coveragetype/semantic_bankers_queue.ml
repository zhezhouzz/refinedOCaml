...
type queue = Queue of int * seq * int * seq
...
let[@refined.predicate] target (q : queue) (size : int) : bool =
  match q with
  | Queue (nf, f, nr, r) ->
      nf = size && length f = nf && length r = nr && nr < nf

let[@refined.logic] front (q : queue) : seq =
  match q with Queue (_, f, _, _) -> f

let[@refined.logic] rear (q : queue) : seq =
  match q with Queue (_, _, _, r) -> r
...
[@@@refined.axiom
{
  ...
  body =
    "implies (target q size) (q = Queue (size, front q, length (rear q), rear \
     q) && length (front q) = size && 0 <= length (rear q) && length (rear q) \
     < size)";
}]
...
let[@refined.coverage
     {
       ...
       universals = [ "size" ];
       witness_relation = "true";
     }] bankers_queue_port (size : int) : queue =
...
let runtime_examples (_unit : unit) =
  target (bankers_queue_port 4) 4 && not (target (bankers_queue_port 0) 0)
