...
let[@refined.over { pre = "true"; post = "result" }]
   [@refined.coverage
     { pre = "true"; post = "result"; witness_relation = "x = 1 && flag" }] read_pair
    (x : int) (flag : bool) : bool =
...
let[@refined.over { pre = "true"; post = "result = x" }] read_nested_box
    (x : int) : int =
...
let[@refined.over { pre = "true"; post = "result = 0 || result = x" }] read_some
    (x : int) : int =
