...
type constructor = { smt_name : string; arguments : sort list }
type datatype = { type_name : string; constructors : constructor list }
...
let contract_of_attributes attributes =
  ...
  | _ ->
      ...
      error ~loc:attribute.attr_loc
        "a binding can have only one refinement contract"
          ...
          (fun parameter ->
            ...
            | Pparam_val _ ->
                error ~loc:parameter.pparam_loc
                  "labelled and optional arguments are not in the first \
                   prototype"
            | Pparam_newtype _ ->
                error ~loc:parameter.pparam_loc
    ...
    (fun declaration ->
      if declaration.ptype_params <> [] then
        error ~loc:declaration.ptype_loc
          "parametric datatype `%s` needs monomorphisation (planned, not \
      ...
      (fun declaration ->
        ...
        let make_constructor ocaml_name arguments =
          let constructor =
            {
              smt_name =
                "C_" ^ smt_identifier owner ^ "_" ^ smt_identifier ocaml_name;
              arguments;
            }
          in
        ...
        | Ptype_record labels ->
            let constructor =
              make_constructor ("record_" ^ owner)
                (List.map
                   (fun label -> sort_of_core_type label.pld_type)
                   labels)
            in
            List.iteri
              (fun index label ->
                Hashtbl.replace fields_table label.pld_name.txt
                  (constructor, index))
              labels;
...
let selector constructor index =
  Printf.sprintf "sel_%s_%d" constructor.smt_name index
...
let lookup ~loc env name =
  ...
  | Some value -> value
  | None ->
      error ~loc "unknown logical variable or unsupported global `%s`" name
...
let rec smt_of_pattern registry env scrutinee pattern =
  ...
  | Ppat_construct ({ txt; _ }, argument) ->
      ...
      let patterns =
        match argument with
        | None -> []
        | Some (_, pattern) -> flatten_pattern pattern
      in
      ...
      let guard, env =
        List.fold_left2
          (fun (guards, env) index pattern ->
            let guard, env =
              smt_of_pattern registry env
                (app (selector constructor index) [ scrutinee ])
                pattern
            in
            (guard :: guards, env))
          ([], env)
  ...
  | _ ->
      error ~loc:pattern.ppat_loc
        "this pattern is not yet supported by the SMT core"
...
let rec smt_of_expression registry env expression =
  ...
  match expression.pexp_desc with
  | Pexp_ident { txt; _ } ->
      lookup ~loc:expression.pexp_loc env (longident_last txt)
  ...
  | Pexp_construct ({ txt; _ }, argument) ->
      ...
      if List.length arguments <> List.length constructor.arguments then
        error ~loc:expression.pexp_loc "constructor `%s` has the wrong arity"
          name;
      if arguments = [] then constructor.smt_name
      else app constructor.smt_name arguments
  | Pexp_apply
      ...
      | None ->
          error ~loc:expression.pexp_loc
            "call to `%s` needs a summary; interprocedural checking is planned"
            name)
  | Pexp_apply
  ...
  | Pexp_match (scrutinee, cases) ->
      ...
      let exhaustive =
        ...
        ||
        let names =
          List.filter_map (fun case -> constructor_name case.pc_lhs) cases
        in
      ...
      if not exhaustive then
        error ~loc:expression.pexp_loc
          "match is not visibly exhaustive; add a catch-all or cover one \
           declared ADT";
      let scrutinee = recurse scrutinee in
      let translate_case case =
        let guard, case_env =
          smt_of_pattern registry env scrutinee case.pc_lhs
        in
        let guard =
  ...
  | Pexp_field (record, { txt; _ }) ->
      ...
      let constructor, index =
        ...
        | Some entry -> entry
        | None ->
            error ~loc:expression.pexp_loc "unknown record field `%s`" field
      in
      ...
      let entries =
        List.map
          (fun ({ txt; _ }, value) ->
            ...
            | None ->
                error ~loc:value.pexp_loc "unknown record field `%s`" field)
          fields
      in
      let constructor =
        match entries with
  ...
  | _ ->
      error ~loc:expression.pexp_loc
        "unsupported expression in refinement core (effects and higher-order \
         values need an explicit theory)"
...
let binder (name, sort) =
  Printf.sprintf "(%s %s)" (smt_identifier name) (smt_sort sort)
...
let datatype_prelude registry extra_sorts =
  ...
  let datatype_sorts =
    List.concat_map
      (fun (datatype : datatype) ->
        List.concat_map
          (fun constructor -> constructor.arguments)
          datatype.constructors)
      registry.datatypes
  in
    ...
    (fun (datatype : datatype) ->
      ...
      List.iter
        (fun constructor ->
          ...
          let constructed =
            if terms = [] then constructor.smt_name
            else app constructor.smt_name terms
          in
          ...
          in
          line "(assert %s)"
            (quantified (app (recognizer constructor) [ constructed ]));
          List.iteri
            (fun index _ ->
              line "(assert %s)"
                (quantified
                   (app "="
                      [
                        app (selector constructor index) [ constructed ];
          ...
          List.iter
            (fun other ->
              if other.smt_name <> constructor.smt_name then
                line "(assert %s)"
                  (quantified
                     (app "not" [ app (recognizer other) [ constructed ] ])))
            datatype.constructors)
        datatype.constructors;
      let value = "v" in
          ...
          let rebuilt =
            let fields =
              List.mapi
                ...
                constructor.arguments
            in
            if fields = [] then constructor.smt_name
            else app constructor.smt_name fields
          in
...
let smt_for_contract registry ~filename mode arguments result_sort body pre post
  ...
  let pre =
    smt_of_expression registry env
      (parse_formula ~filename ~loc:Location.none pre)
  in
          ...
          List.filter_map
            (fun binding ->
              ...
              | Some (mode, pre, post) ->
                  ...
                  in
                  let arguments, result_sort, body =
                    function_shape binding.pvb_expr
                  in
                  Some
                    {
                      name;
                      ...
                      smt =
                        smt_for_contract registry ~filename mode arguments
                          result_sort body pre post;
                      trusted_axioms = [];
                    })
            bindings
      | _ -> [])
...
(* Typedtree frontend -----------------------------------------------------
   ...
   The legacy source frontend above remains available for the initial
   prototype.  The MVP path below consumes the Typedtree emitted by OCaml,
   so all names and ordinary types have already been resolved by the real
   compiler. *)
